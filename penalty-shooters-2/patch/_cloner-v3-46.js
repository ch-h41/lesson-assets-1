...
(function () {
        ...
        process: function () {
            while (this.active < this.limit && this.queue.length > 0) {
                ...
                try {
                    ...
                    console.error("RequestQueue process error", e);
                    this.release(); // ensure release if fn crashes synchronously, though fn is usually a wrapper
                }
            }
        },
        release: function () {
            this.active--;
            if (this.active < 0) this.active = 0; // Safety
            this.process();
        }
    };
    ...
    XMLHttpRequest.prototype.send = function (body) {
        const self = this;
        // Wrap the original send in the queue
        RequestQueue.add(() => {
            // We need to know when the request finishes to release the queue slot
            // 'loadend' fires when the request has completed (whether successfully (after 'load') or unsuccessfully (after 'abort' or 'error')).
            let released = false;
            const release = () => {
                if (released) return;
                released = true;
            ...
            self.addEventListener('loadend', release, { once: true });

            // In case loadend doesn't fire for some reason (though it should),
            // we could perform a safety check, but loadend is standard.
            // Just to be safe for re-entrance or weird states:
            try {
                origSend.call(self, body);
            } catch (e) {
                // sync error, release immediately
                release();
                throw e;
            }
        });
    ...
    Object.defineProperty(HTMLImageElement.prototype, 'src', {
        ...
        set(v) {
            const self = this;
            const patchedUrl = URLPatch(v, "IMG-OBJ-SRC-SET", "IMG");
            RequestQueue.add(() => {
                let released = false;
                const release = () => {
                    if (released) return;
                ...
                self.addEventListener('load', release, { once: true });
                self.addEventListener('error', release, { once: true });
                try {
                    origHTMLImageElementSrc.set.call(self, patchedUrl);
                    if (!patchedUrl) release(); // Handle empty src immediately
                } catch (e) {
                    release();
                    throw e;
                }
